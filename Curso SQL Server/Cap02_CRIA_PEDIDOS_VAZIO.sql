...
ADD CODCLI int Not Null
	Constraint FK_TB_PEDIDO_TB_CLIENTE
	Foreign Key (CODCLI)
	References TB_CLIENTE(CODCLI);
...
ALTER TABLE TB_PEDIDO
ADD CODVEN smallint Not Null
	Constraint FK_TB_PEDIDO_TB_VENDEDOR
	Foreign key (CODVEN)
	References TB_VENDEDOR(CODVEN);

ALTER TABLE TB_PRODUTO
DROP COLUMN COD_TIPO;

ALTER TABLE TB_PRODUTO
ADD COD_TIPO smallint not null
	Constraint FK_TB_PRODUTO_TB_TIPOPRODUTO
	Foreign key(COD_TIPO)
	References TB_TIPOPRODUTO(COD_TIPO);


ALTER TABLE TB_PRODUTO
DROP COLUMN COD_UNIDADE;

ALTER TABLE TB_PRODUTO
ADD	COD_UNIDADE smallint not null
	constraint FK_TB_PRODUTO_TB_UNIDADE
...
ADD ID_PRODUTO INT NOT NULL
	CONSTRAINT FK_TP_ITENSPEDIDO_TB_PRODUTO
...
ALTER TABLE TB_UNIDADE ADD CONSTRAINT CHUNICA_UNIDADE UNIQUE (UNIDADE);

ALTER TABLE TB_TIPOPRODUTO ADD CONSTRAINT CHUNICA_TIPO UNIQUE (TIPO);

ALTER TABlE TB_ITENSPEDIDO 
DROP COLUMN QTD_REAL;

USE PEDIDOS_VAZIO;


ALTER TABLE TB_PRODUTO
ADD QTD_REAL INT NOT NULL
	CONSTRAINT CK_TB_PRODUTO_QTD_REAL
	CHECK(QTD_REAL > 0);
...
ADD QUANTIDADE INT NOT NULL
	CONSTRAINT CK_TBITENSPEDIDO_QUANTIDADE
	CHECK(QUANTIDADE > 0);

ALTER TABLE TB_ITENSPEDIDO ADD CONSTRAINT CK_TBITENSPEDIDO_PR_UNITARIO CHECK(PR_UNITARIO >= 0);

ALTER TABLE TB_ITENSPEDIDO ADD CONSTRAINT CK_TB_ITENSPEDIDO_DESCONTO CHECK(DESCONTO >= 0 AND DESCONTO <= 10);


ALTER TABLE TB_PRODUTO ADD DEFAULT 0 FOR COD_UNIDADE;
